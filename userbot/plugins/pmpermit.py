...
# make by legendx22 for legend bot
...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "DÃ¨vÃ­láºÃ¸t UsÃªr"
...
USER_BOT_NO_WARN = (
    "Hello, This is **LEGEND BOT ULTRA Security Protocolâ ï¸**.\n"
    f"This is my master {DEFAULTUSER}'s Inbox\n"
    f"\n**{KRAKEN}**\n\n"
    "To start a valid conversation\nð±Register Your Request!ð±\nSend `/start` To Register Your Request\nHopefully u will get a replyð¥"
)
        ...
        if event.is_private:
            if not pmpermit_sql.is_approved(chat.id):
                if not chat.id in PM_WARNS:
                    ...
                    bruh = "__Auto-approved bcuz outgoing ð¶__"
    ...
    async def approve_p_m(event):
        ...
        if event.is_private:
            if chat.id == 1100231654:
                await event.edit(
                    "You tried to block my masterð¡. GoodBye for 100 seconds!ð¥±ð´ðªð¤"
                )
                time.sleep(100)
            else:
    ...
    async def approve_p_m(event):
        ...
        if len(approved_users) > 0:
            for a_user in approved_users:
                if a_user.reason:
                    APPROVED_PMs += f"ð [{a_user.chat_id}](tg://user?id={a_user.chat_id}) for {a_user.reason}\n"
                else:
                    APPROVED_PMs += (
                        f"ð [{a_user.chat_id}](tg://user?id={a_user.chat_id})\n"
                    )
        else:
    ...
    if event.is_private:
        if not pmpermit_sql.is_approved(chat.id):
            pmpermit_sql.approve(
                chat.id, "**My Boss iz here.... It's your lucky day nibbað**"
            )
            await borg.send_message(chat, "**Here comes my Master! Lucky you!!ð**")

CMD_HELP.update(
    {
        "pmpermit": "**Plugin : **`pmpermit`\
    \n\n**Syntax : **`.allow .disallow .block .unblock .listallowed`\
    \n**Function : **not info added sry"
    }
)
