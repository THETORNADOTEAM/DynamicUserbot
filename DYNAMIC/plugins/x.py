...
# 🇨 🇭 🇦 🇱 🇴      🇸 🇮 🇷    🇵 🇱 🇪 🇦 🇸 🇪    🇬 🇪 🇹 🇴 🇺 🇹    
  ...
  #     
    ...
    async def inline_handler(event):#modify by DYNAMICx22
#credits team dc
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query.startswith("Userbot") or event.query.user_id == id and query.startswith("Userbot"):
            ...
            await event.answer([result] if result else None)
              ...
              reply_pop_up_alert = "Please get your own Userbot,for more info visit @DYNAMICUSERBOTSUPPORT! "
              await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
 ...
 #hehehehehhehhehhehe   
    ...
    async def on_plug_in_callback_query_handler(event):
        if not event.query.user_id == bot.uid or event.query.user_id == id:
            atul= "Please get your own Userbot,for more info visit @DYNAMICUSERBOT! "
        ...
        try:
            if event.query.user_id == bot.uid or event.query.user_id == id:
                ...
                reply_pop_up_alert = "Please get your own Userbot, and don't use mine for more info visit @DYNAMICUSERBOTSUPPORT!"
            ...
            if event.query.user_id == bot.uid or event.query.user_id == id:
                ...
                reply_pop_up_aTEAMISHEREease get your own Userbot, and don't use mine for more info visit @DYNAMICUSERBOTSUPPORT!"
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
...
#  🇦 🇷 🇪      🇧 🇸 🇩 🇰      🇮 🇸 🇸 🇪    🇰 🇦 🇳 🇬  🇲 🇦 🇹   🇰 🇷    🇷 🇪   🇲 🇨 
                    ...
                    try:
                        ...
                        await event.edit( "`Your DYNAMIC BOT Has Successfully loaded` >>>" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
                    except Exception as e:
                        await event.edit("Error{}".format(shortname, str(e))+ "DYNAMIC BOT Has Successfully loaded" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
    ...
    async def on_plug_in_callback_query_handler(event):
              if event.query.user_id == bot.uid or event.query.user_id == id:
                    ...
                    try:
                        ...
                        await event.edit( "`Your DYNAMIC BOT Has Successfully unloaded` >>>" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
                    except Exception as e:
                        await event.edit("Error{}".format(shortname, str(e)) +"DYNAMICBOT Has Successfully unloaded"+ str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
                    shortname = event.data_match.group(1).decode("UTF-8")
                    fcix = [custom.Button.inline("  𝕭𝖆𝖈𝖐 ",data="back({})".format(shivam)),custom.Button.inline(" 𝕮𝖑𝖔𝖘𝖊 ", data="close"),custom.Button.inline(" 𝕷𝖔𝖆𝖉 ",data="load({})".format(shivam_sh1vam))]
                    reply_pop_up_alert = "Please get your own Userbot,for more info visit @DYNAMICUSERBOTSUPPORT!"
