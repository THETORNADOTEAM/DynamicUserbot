...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"**DYNAMIC Sᴜᴘᴇʀғᴀʙɴ Cᴏᴍɪɴɢ Oɴ Oʀᴅᴇʀ Oғ {PRO}**...")
    fedList = []
    ...
    if not len(fedList):
        ...
        if len(fedList) == 0:
            ...
            return
    await event.edit(f"DYNAMIC Fʙᴀɴɴɪɴɢ Tʜɪs Kɪᴅ Iɴ **{len(fedList)}** Oɴ Tʜᴇ Oʀᴅᴇʀ Oғ **{PRO}** 🔥.")
    try:
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit(f"**DYNAMIC ѕтαятιηg ѕυρєяυηƒвαη ση σя∂єя σƒ {PRO}**...")
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
        FBAN = previous_message.sender_id
    else:
    ...
    for fed in fedList:
        ...
        await asyncio.sleep(5)
    await event.edit(f"DYNAMIC UnFBan Completed. Affected {len(fedList)} Feds by {PRO}.\n#DYNAMICX Userbot")
...
CMD_HELP.update(
    {
        ...
        \n\nGet help - @DYNAMICUSERBOTSUPPORT
    }
)
